...
class CmsSettings extends CApplicationComponent
{
	...
	{        
		$connection=$this->getDbComponent();
		$command=$connection->createCommand('SELECT * FROM '.$this->getTableName());
		$result=$command->queryAll();
		...
		$data = array();
		foreach ($result as $key => $sub_name) {
			foreach ($sub_name as $key2 => $value) {
				if ($sub_name['category']==$value) {
					$data[$value][$sub_name['key']]=$sub_name['id'];
					$data[$value][$sub_name['key']]=$sub_name['category'];
					$data[$value][$sub_name['key']]=$sub_name['key'];
					$data[$value][$sub_name['key']]=$sub_name['value'];
				}
			}
		}
